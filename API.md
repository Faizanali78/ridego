...
Payments are never captured from a client-side boolean. UPI/card-style payments require a server order and backend provider verification before `paymentStatus` becomes `completed`. If `STRIPE_SECRET_KEY` is configured, `/api/payments/orders` creates a Stripe Checkout Session and `/api/payments/verify` confirms the returned session with Stripe. Razorpay credentials remain supported as a fallback; otherwise the local signed gateway is used for offline demos/tests. Wallet changes are stored as a ledger in `walletTransactions` with `credit`/`debit`, amount, reason, reference, optional `rideId`, balance-after, and timestamp. Driver ride earnings are stored separately in `driverEarnings` with ride fare, platform fee, and net driver earning.
